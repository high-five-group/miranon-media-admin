...
- [ ] **ADR-043-bygge — inkrement 4–5** (carry) — per [ADR-043](../docs/decisions/ADR-043-session-lifecycle-skills-arkitektur.md) Implementationsplan: handoff-kontrakt (formalisera Chat→Code-direktiv som template-fil, spegel till code-role-discipline §4) + discovery-/dogfood-test (`/`-skills laddar + kör; session-resume rekonstruerar mitt-i-sessionen).
