...
*Senast uppdaterad: 2026-07-04 (**Session 49 ✅ AVSLUTAD** (lifecycle: closed; session 50 öppnas via /session-start) — **FORK 4 BYGGD END-TO-END:** hub-plugin 1.7.0 (/to-prd + /to-issues + delad referens issue-substrat.md + NÄR-rad ISSUE-SUBSTRAT; hub e16add2) + spoke-substratet konfigurerat (DoD-defaults 4 poster + labels-deklaration 3 + demo-korten arkiverade + systemet.md §0-post + grind-legibility-rader på 5 ytor; L225-grinden prövad med tomt fixtur-utfall). Lessons **L228–L229** skördade + hub-lyfta som K49.1–2 (hub `f0e62f2`); T60 routad till migrerings-hub-session 1 (additiv not i tråd-registret, `paused` orört). Spoke-trail: 28bc365 · ca4482d · e8c8d93 + avslutscommit; CI-runs 28716759005 / 28717976844 / 28718721162 gröna per jobb (sista med Test + Build KÖRDA). **NÄSTA SESSION 50 — INTEGRATIONEN SLUTFÖRS FÖRST** (Marcus-direktiv, rättelse 2026-07-04; beroendekedjan T57 → fork 4 → do-work per S47 Del 16 F1 + S48 DEFER-karta): **do-work-landningen ÖVERST och ENDA huvudkandidat** — grillning via /grill-with-docs (prövotids-dp9) mot code-role-disciplinen + issue-substrat.md + Backlog.md-CLI:t; TDD-adaptionen (Del 15) + etikett-driven plockning som operativ kö ingår i grillningens grenar; bygge → hub-plugin 1.7.0→1.8.0. DÄREFTER S51: UI-spårets start = integrationens slutstation i drift — första skarpa hel-kedje-körningen /to-prd → /to-issues → do-work på riktig arbetsenhet (triggar fork 5+6-byggena [låst trigger] + första drift-metrik-matningen ADR-068 p.8). Inga produkt- eller sidospår före do-work. Städ-/mognadsskikt därefter: migrerings-hub-session 1 (inkl. T60 + invokerings-UX) / T56 / L193–L222-lyftet. Count 67/67/67; ADR-068 WIP; mail sövt (T51/T53/T55); Fas 6-closeout-förkrav (T38/T39/T40) bärs vidare; L193–L222-lyft PENDING.)*
