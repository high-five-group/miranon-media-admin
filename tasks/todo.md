...
- [ ] **ADR-043-bygge — inkrement 5 (discovery-/dogfood-test)** (carry) — per [ADR-043](../docs/decisions/ADR-043-session-lifecycle-skills-arkitektur.md) Implementationsplan: verifiera att `/session-start`, `/session-end`, `/session-resume` laddar och kör korrekt end-to-end; bekräfta att session-resume rekonstruerar mitt-i-sessionen-läge ur sessionsdok + todo + BUILD-LOG.
