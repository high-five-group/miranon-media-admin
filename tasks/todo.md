...
*Senast uppdaterad: 2026-07-04 (Session 49 🔄 AKTIV — **fork 4-bygget: sessionsdok FÖTT via /session-start (skapande-gren ADR-043; Del 1-scope verbatim låst; födelse-commit + todo-kadens denna landning).** Ordnad scope: (1) bygg-precisering — PRD-kort-mall + skiv-/issue-mall som Backlog.md-flöden (hierarkiska barn-ID:n) + DoD-defaults (`definition_of_done`) + etikettvokabulär (triage-labels-mappning inkl. ready-for-agent) + kö-ordning (Del 12 vii) + agent-instruktionsrad (NÄR-rad-klass) + QA-/uppföljningsmönster; (2) hub-landning — adapterade to-prd/to-issues + mall-bärare, plugin-bump 1.6.0→1.7.0 antaget (L55-cache-ritual + omstarts-verifiering); (3) spoke-landning — config.yml DoD-defaults + demo-kort-städning (task-1/task-1.1) + systemet.md §0-post + ev. L225-grindrader; (4) beslutspunkter Marcus — export-path-quirk-uppströms + inget skarpt PRD-kort utan explicit beslut; (5) om utrymme — T60-triagen. EJ i scope: do-work-grillningen / TDD-adaptionen / etikett-kö + AFK-loop / migrerings-exekveringen / T56 / mail-go-live / UI-spåret / ADR-068-gradering / L193–L222-lyftet. Ingång disk-verifierad: count 67/67/67 (ADR-068 WIP); nästa lesson L228; spoke 987d962 rent/i synk, CI-run 28715831650 grön per jobb; hub f573efd, plugin 1.6.0 (8 skills); backlog-substratet exakt per S48 Del 3 (config-flaggor false, ingen definition_of_done-nyckel, demo-korten på plats). Mail sövt (T51/T53/T55); UI deferrat; L193–L222-backloggen PENDING; Fas 6-closeout-förkrav (T38/T39/T40) bärs vidare. Tidigare: Session 48 ✅ AVSLUTAD — T57 END-TO-END LEVERERAD & STÄNGD: Backlog.md valt (grillning dp8, 8/8 defektfria; DECLINE L30-durabla inkl. GitHub Issues-moln + noll-option) + minimal-test 8/8 grönt + substrat LIVE (`backlog/` v1.47.1, demo-kort task-1/task-1.1) + T60 registrerad; lessons L226–L227 skördade + hub-lyft L223–L227 (hub f573efd).)*
