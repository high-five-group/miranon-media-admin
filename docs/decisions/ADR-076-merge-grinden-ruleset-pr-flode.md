...
>
> **Korrigering (2026-07-27, S91 tredje resumen — ägarformen har flyttat sig):** Kontext-styckets mening "Repot är publikt (ADR-024) och User-ägt — rulesets är kostnadsfritt tillgängliga; merge queue är det INTE (kräver org-ägt repo)" gäller inte längre. Marcus överförde repot till organisationen `high-five-group` (skapad 2026-07-27 kl 11:15:43Z) på **enterprise-plan**, uttryckligen för att öppna merge queue; verifierat `plan=enterprise`, ägartyp `Organization`, synlighet `public`. **Merge queue är från detta datum tillgänglig.** Detta beslut i övrigt är OFÖRÄNDRAT: `main-skydd` med PR-krav 0 approvals, required check `CI Passed or Skipped` strict och tom bypass-lista står kvar, och rulesetet verifierades intakt genom överföringen (`enforcement=active`) tillsammans med de 8 Actions-secreterna och `marcus803`:s assignable-status (ADR-077:s larmkedja). **Att merge queue är tillgänglig är inte skäl att aktivera den:** [research-passet 2026-07-26](../research/merge-queue-mot-staging-mutex-2026-07-26.md) § Om vi ändå flyttade repot visar att bästa fallet är oförändrade ≈27 min och att default-fallet är ≈55 min — dubblering — om det tunga jobbet inte villkoras på `github.event_name`. Aktivering är ett öppet Marcus-beslut som hör till CI-/grind-arkitekturens eget pass; se amenderingsblocket i research-passet för vad som faller och vad som står.
>
> **Amendering (2026-08-05, S97 fjärde resumen — `strict` AVSTÄNGD, den blev redundant när kön aktiverades):** Beslutets `strict_required_status_checks_policy: true` (rad 43–44, 82) står inte längre. **Skälet är att merge queue aktiverades 2026-07-29 (`TASK-70.1`, A7:3) och gör strict redundant — förstapartskällan säger det rakt ut:** *"The merge queue provides the same benefits as the **Require branches to be up to date before merging** branch protection, but does not require a pull request author to update their pull request branch and wait for status checks to finish before trying to merge"* ([managing-a-merge-queue](https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/configuring-pull-request-merges/managing-a-merge-queue)). Kön bygger varje post mot `main` plus posterna före den — exakt den garanti strict gav. **Redundansen var inte harmlös, den var en deadlock.** Strict kräver uppdaterad gren som VILLKOR FÖR ATT POSTEN ENS FÅR KÖAS; en PR som blev `BEHIND` innan den hann köas släpptes därför aldrig in, och kön fick aldrig chansen att sekvensera den. Mätt skarpt samma dag: `#747` och `#748` stod `BEHIND` med `auto=true` och rörde sig inte förrän `gh pr update-branch` kördes för hand på båda — en `T108`-klass, tillstånd utan bevakare. **Konsekvens-sektionens rad "Strict up-to-date serialiserar PR-landningar … vid parallella PR:er krävs branch-uppdatering innan auto-merge fyrar" (rad ~159) beskrev alltså detta korrekt som en ACCEPTERAD kostnad — den skrevs 2026-07-23, sex dagar före kön fanns, och konsumerades aldrig när förutsättningen ändrades.** Åtgärd: `strict_required_status_checks_policy` satt till `false` via `gh api -X PUT` på ruleset `19627609`; diffen mot före-tillståndet var exakt en rad, och samtliga fem regler (`deletion`, `non_fast_forward`, `pull_request`, `required_status_checks`, `merge_queue`) verifierades kvar efteråt. Före-tillståndet sparat före ändringen. **Vad som INTE ändrades:** required check `CI Passed or Skipped` är kvar och gäller oförändrat, PR-kravet med 0 approvals står, bypass-listan är fortsatt tom. **Vad detta beror på:** garantin bärs nu av `merge_queue`-regeln ensam. Rivs den regeln någon gång — t.ex. via nödvägen i `CONTRIBUTING.md` § Landnings-ordningen — måste `strict` sättas tillbaka i samma andetag, annars försvinner up-to-date-skyddet helt.
