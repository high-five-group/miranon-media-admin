...
>
> **Korrigering (Session 22, 2026-06-17 — se [ADR-054](ADR-054-fetch-depth-full-historik.md)):** `fetch-depth` + `FRONTMATTER_MIN_HISTORY_DEPTH` + `check-frontmatter.sh`-default är ändrade `250 → 0` (full historik). Finit djup föll en fjärde gång (Session 22): en dok-commit sköt fönstret `263 → 264` och tre orörda governing-docs (SECURITY-SPEC, hur-systemet-funkar, data-model) fälldes på falsk drift då boundary-committen blev proxy för deras sanna commit-datum. `0` = hela historiken; Check 2 läser då alltid filens äkta `git log`-datum. Konsekvens: shallow-detektionen (count < 0) blir per design en no-op (acceptabelt — kanonisk config är full-clone; apparatens avveckling spåras av tråd T08). Truth-table-kommentarerna i `.frontmatter-policy.conf` + `check-frontmatter.sh` + T10/T11b-edge-värdena är uppdaterade i synk. Beslutstexten + tidigare errata nedan bevaras oförändrade (öppen rivning med kvittens).
