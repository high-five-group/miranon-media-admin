...
>
> **Korrigering (Session 22, 2026-06-17 — se [ADR-054](ADR-054-fetch-depth-full-historik.md)):** `changed`-jobbets `fetch-depth` är ändrat `250 → 0` (full historik). Detta är inte en femte bump utan en falsifiering av finit-djup-rationalen: mönstret brast en fjärde gång (Session 22, en dok-commit sköt fönstret `263 → 264` och fällde tre orörda governing-docs på falsk drift). Värdet `0` = hela historiken är det kanoniska svaret för git-log-historik-beroende checks. Beslutstexten + tidigare errata nedan bevaras oförändrade (immutabilitet, öppen rivning med kvittens).
>
> **Korrigering (Session 77, 2026-07-23 — utelämning #3 STÄNGD, se [ADR-076](ADR-076-merge-grinden-ruleset-pr-flode.md)-vågen):** actionlint installeras inte längre via maintainer-distribuerat download-script från muterbar `main` — processgranskningen 2026-07-23 § 8 visade att skriptets egen checksumme-verifiering inte skyddar mot komprometterad skript-källa (skriptet exekverar före all verifiering). Ny form (PR #99): release-pinnad version + downstream-beräknad SHA256 verifierad i workflowen — shellcheck-stegets redan etablerade husmönster. Utelämningens motivering ("Vite gör det") var giltig 2026-05-13; huset växte förbi den. Beslutstexten nedan bevaras oförändrad.
>
> **Korrigering (Session 77, 2026-07-23 — utelämning #5 STÄNGD, se [ADR-076](ADR-076-merge-grinden-ruleset-pr-flode.md)):** branch protection är AKTIVERAD som ruleset `main-skydd` med `ci-passed`-aggregatorn ("CI Passed or Skipped") som required check — exakt den framtida aktivering som utelämning #5 och § Konvention förutsåg. Aggregator-namnet förblir required-check-stabilt. Deferralen stod 2026-05-13 → 2026-07-23 utan återbesöks-trigger; klassen skördas som lesson i S77. Vidare är Test+Build-jobbet splittat i `test-fast`/`a11y`/`test-staging` (PR #99, S77) — § 1c:s enjobbs-form är därmed superseded i topologi (skip-mönstret + aggregatorn består); fetch-depth-bärarna i ci.yml är 3 (changed/lint/docs), se [ADR-039](ADR-039-konsistens-grindar-kadens.md)-erratum.
>
> **Korrigering (Session 91, 2026-07-28 — utelämning #2 RIVEN, se [ADR-082](ADR-082-lankgrindens-form-presubmit-postsubmit.md)):** `.lycheeignore`:s **add-only-policy är upphävd**. Motiveringen ("preventiv exklusion tystar K18-signal vi inte vet om är problem") var giltig så länge filen var ett *grind-tystare* — varje rad tog bort en PR-blockerare. Sedan ADR-082 kör `ci.yml`:s docs-jobb med `--offline` och ser inga externa adresser alls, så en rad i filen kan inte längre tysta någon blockerande grind; den håller en nattrapport läsbar. Filen bytte därmed roll till **brusfilter**, och policyn ersätts av ett **motiveringskrav** (varje post bär skäl + datum) som gäller åt båda håll: poster får läggas till utan bevisad flakiness, och de får tas bort. Empirin som fällde den gamla formen: undantagslistan växte till 22 mönster varav 21 externa, och grindens sprängradie var 467 externa länkar på **125 skilda värdar** — var och en kunde ensam rödmåla en PR som inte rörde den. Av nio undersökta branschledande projekt låter noll externa länkar blockera en PR. Samtidigt faller § Konsekvenser-raden *"broken links åtgärdas som drift, inte tystas via preventiv `.lycheeignore`"* för **extern** yta; för intern yta står den kvar oförändrad och grindas fortsatt blockerande. Beslutstexten nedan bevaras oförändrad (immutabilitet, öppen rivning med kvittens).
