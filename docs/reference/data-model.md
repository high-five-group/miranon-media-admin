...
Edge Functions lever i `~/Repon/psionautics/supabase/functions/` och deployas automatiskt via Lovable. De agerar som webhook-konsumenter (POST från admin-frontend) och läser/skriver mot Airtable med en serverside-sparad `AIRTABLE_TOKEN`.

10 funktioner finns i mappen, varav 7 rör Airtable. De 3 övriga (`generate-template-image`, `create-admin-user`, `get-plausible-stats`) är ej Airtable-skrivande och dokumenteras kort sist.

För Miranon Media-appen: 4 implementerade, 8 planerade i `miranon-media-admin/src/data/adapters/AirtableAdapter.ts` (TODO).

### Översikt — alla 10 funktioner

| Function | Senaste commit | Datum (UTC) | Tabeller skrivs till | Tabeller läses från |
|---|---|---|---|---|
| `create-registration` | `22b8692` | 2026-04-15 09:00 | Anmälningar | Anmälningar (dubblett-check) |
| `create-waitlist-entry` | `7585604` | 2026-04-15 08:55 | Väntelista | Väntelista (dubblett-check) |
| `update-registration` | `811adbf` | 2026-04-15 09:12 | Anmälningar, Eventplanering | – |
| `get-event-bookings` | `6a7c9a9` | 2026-04-16 07:51 | – (read-only) | Anmälningar, Eventplanering |
| `get-waitlist-stats` | `3e7eeee` | 2026-02-21 12:48 | – | Väntelista |
| `get-waitlist` | `30bd2c9` | 2026-04-27 08:40 | – | Väntelista |
| `send-email` | `1a07d1b` | 2026-04-27 08:39 | Anmälningar, Väntelista | – |
| `generate-template-image` | (mar 2026) | 2026-03-20 | – | – (ScreenshotOne) |
| `create-admin-user` | (mar 2026) | 2026-03-20 | – | Supabase Auth (ej Airtable) |
| `get-plausible-stats` | (mar 2026) | 2026-03-20 | – | Plausible API (pausad) |

Källa: `git log` per fil 2026-04-28 + `01-extraction.md` §I.
...
```ts
{
  ...
  email?: string,         // normaliseras: trim + lowercase
  telefon?: string,
  notering?: string,
  ...
  medfoljandeTill?: string  // record-ID → Medföljande till (self-link)
}
```
...
- A11 körs på nya Deltaganden → sätter Person-länk

**Note:** Funktionen är hårdkodad för MK-eventet. För framtida event behövs antingen ny Edge Function eller parameterisering.
...
**Dubblettcheck:** Sök Väntelista med `{E-post}=$email` → om hit returnera **409** med `{ error: 'duplicate', existingName }`.

**Felfall:** 400 (saknade fält), 409 (dubblett), 500 (AIRTABLE_TOKEN saknas eller Airtable-fel — returnerar `detail` med Airtable-felmeddelande för debug).

**Antaganden:** Inga automationer triggas automatiskt på Väntelista-creates idag — A4/A5 triggar bara på Hämtade erbjudanden, inte Väntelista. Endast manuell flytt till Anmälningar via UI (se §F.4).
...
**Read-only.** Aggregerar Väntelista per `utm_content` / `utm_medium` / `utm_source` / `utm_campaign`. **Inte ändrad sedan februari 2026** — utanför scope för 2026-04-arbetet.

### `send-email` — den centrala mail-Edge Function
...
| `type` | Resend-mall | Variabler |
|---|---|---|
| `confirmation` | `medveten-kontakt-bekraftelse` | `{{{name}}}` |
| `payment` | `medveten-kontakt-betalning` | `{{{name}}}` |
| `plus_one` | `medveten-kontakt-plus-one` | `{{{name}}}` |
| `participant-info` | `medveten-kontakt-deltagarinformation` | `{{{name}}}`, `{{{pdfUrl}}}` (rich-text-länk i mall, inte markdown) |
| `waitlist-info-1` | `medveten-kontakt-vantelista-info-1` | `{{{name}}}` |

#### `patchAfterSend(type, recordId, token)` — Anmälningar

Anropas av dispatcher när `type` inte börjar med `waitlist-`.

| `type` | Sätter på Anmälningar |
|---|---|
...
#### `patchWaitlistAfterSend(type, recordId, token)` — Väntelista

Anropas av dispatcher när `type` börjar med `waitlist-`.

| `type` | Sätter på Väntelista |
|---|---|
...
**Tyst fail-mode:** PATCH-misslyckande loggas (`console.error('Post-send PATCH failed: ...')`) men funktionen returnerar **fortfarande ok**. Mail har skickats; UI tror det misslyckades. Se §Kända fällor 29 och §F.3.

### Övriga Edge Functions (ej Airtable-skrivande)

- **`generate-template-image`** — POST. Tar emot `{ url, filename, width, height }` → ScreenshotOne-anrop → returnerar PNG-binär. Används av admin Promo Kit-flik. Inga Airtable-IDs.
- **`create-admin-user`** — Skapar admin-konton i Supabase Auth. Inga Airtable-IDs.
- **`get-plausible-stats`** — Plausible API-proxy. **Pausad** sedan april 2026 (Plausible-prenumeration löpt ut).

Källa: `01-extraction.md` §I + `git log` per fil 2026-04-28.
