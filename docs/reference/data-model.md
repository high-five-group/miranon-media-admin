...
46. **`Motivering (sammanfattning)` (`fldrMT8cWP3NmBc9T`) deklarerar `result: singleLineText` men API:et returnerar en ARRAY — schemat motsäger sin egen utdata. [LIVE-BEKRÄFTAD 2026-07-30 — Session 91, `TASK-89`]** Formelns ELSE-gren returnerar rollup-referensen **orörd** i stället för att platta den till en sträng, medan fältet samtidigt deklarerar `singleLineText`. Personhämtningen fäller därför i schemavalideringen: `get-person/index.ts:128` skickar EF-svaret med arrayen orörd (HTTP **200**), `AirtableAdapter.ts:131` lämnar vidare, och `PersonDetail.schema.ts:44` avvisar med `code=invalid_type`, `expected string, received array`. **Isolerat i båda riktningar:** Greta (`rec2ChwRvXAjwdr4m`) och Frida (`recw3SNa4ulwSN3tZ`) fäller · en person utan motivering (`recqxaFNwHAdQlAqb`) går grön · Greta med *enbart* `motivering` plattad till sträng går grön med **noll kvarvarande issues**. ⚠️ **`TASK-52`:s ursprungliga diagnos var FEL i motsatt riktning:** noten sade *"array när flera anmälningar finns"*, men båda de observerade personerna har `Antal anmälningar (totalt)` = **1**. Arrayformen uppstår vid den **första** motiveringen — antalet anmälningar är irrelevant. **Omätt led:** flerhet (>1 element) är inte observerad någonstans, så varje rekommendation om "bevarad flerhet" vilar på ett antagande. **Tre syskonfält bär samma formelmönster** — strukturell misstanke, ej belagd. → **Maximerings-kandidat (T16):** platta ELSE-grenen i formeln så deklarerad typ och faktisk utdata överensstämmer, alternativt deklarera fältet som det den faktiskt är. Resolution I BASEN ([ADR-063](../decisions/ADR-063-airtable-bas-som-forstklassig-leverabel.md)), ej app-lapp — appen ska inte lära sig acceptera en typ basen påstår att den inte skickar.
