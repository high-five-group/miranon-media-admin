...
- **FJS (Event-18) + RIM1 (Event-19)** (pre-paus): 20 walk-in-anmälningar (Marcus, Scripting), 2 kantfall fixade via MCP (Deltagare 42 reverse-flow; Deltagare 49 case-e-post-dubblett), **80 Deltaganden → Närvarande**, verifierat cross-event (Deltagare 05 4/4).
...
- **Full narrativ:** `tasks/sessions/archive/2026-08/2026-08-21-session-110.md` Del 1–13 + fyra paushistoriker; beslut `docs/decisions/ADR-122-eventlankens-vakt-och-atgardskon.md`; vaktskriptet `docs/reference/automation-scripts/a1-eventmatchning-vakt.js`; Lotta-underlaget `~/Downloads/lotta-underlag-anmalningar-2026-08-21.md` (utanför repot)
...
## Session 123 (2026-09-06 → 2026-09-07) — Lottas prodläge: skeleton-krom, förvärmning, bilagor på åtgärder, prod-sekvensen för TASK-367

- **Commit-range:** `29a3c16d` (prod vid start) → `c357e3ee` (`#2453`, TASK-432, sista kodlandningen) plus stängnings-PR:n. En kontrollerad kompaktering (ADR-101), en paus (AFK-natt) och en resume.
- **Mål:** Marcus fråga inför Lottas start: exakt prodläge, skeleton på rätt ställen och bilagor utan väntan; sedan i resume 1 landa allt som stod i luften, köra prod-sekvensen för TASK-367 och stänga sessionen autonomt.
- **Faktiskt:** fem read-only-rapporter mot kod och leverantörer → research "förvärm inte allt" (`docs/research/forvarma-allt-branschmonster-2026-09-06.md`) → PRD `TASK-416` med 21 skivor, 19 landade genom review-grinden (416.15 QA kvar) → TASK-367 landad (`#2416`, `988e0d3b`) med migration i prod på Marcus diktering, fas4-deploy av Marcus (57 EF:er) och staging-omdeploy för paritet → Lotta-synlig regression i bilagenamn (TASK-361 r2) hittad, rotorsakad och rättad samma kväll (`#2452`, `790650d5`) → Airtable-prod-låset för agenter (`#2442`, `51144a3c`, Marcus 419 A) → nightlys tre docs-grindar gröna (424–426) → CLAUDE.md § Verktygsfakta bär det nya låset.
- **Avvikelser:** staging före main i medKvitto-kravet tills `#2416` landade · sentinel-driften L599 fällde tre post-merge (städad via MCP) · esm.sh saknade supabase-js 2.116.0 minuter efter publicering och fällde första fas4-körningen (`TASK-433`) · tre granskare tappade JSON-blocket · sex premissfel i uppdragstexter fångade av mottagare (ADR-086) · `#2448` DIRTY på fyra kortkonflikter.
- **Verifiering:** varje kod-PR genom review-agent i färsk kontext (13 PR:er, 8 i runda 1, 3 i runda 2, 5 exit 20 varav 2 till Marcus) med loop-beslut, sektion och backstopp-preflight; prod mätt med `--kontrollera` före/efter; Vercel prod verifierad via GitHub Deployments; post-merge gröna från `fb814734`.
- **Teknisk skuld / Marcus-moment:** 416.15 QA · 403 beslut (läs/skriv) · 423 · skarpbevis för `deny-prod-airtable.sh` · TASK-430 (agent-grant) · TASK-433 (pinning, HIGH) · L599 motmedel (a) · Roger DNS + `miranon.se` 2026-10-18.
- **Full narrativ:** `tasks/sessions/2026-09-06-session-123.md` Del 1–6 + Paushistorik; sex lessons-fragment i `tasks/lessons.d/` (tre `[UNIVERSAL]`); L599 tredje instansen i `tasks/lessons/vol-08.md`.

## Session 124 (2026-09-08) — Lottas prodläge II: plockaren fixad, betalningsblocket rivet, eventdetaljens Öppna detaljer i ny form; avslutad administrativt av S125

- **Commit-range:** `a0320b33` (S123-stängning, prod vid start) → `e4c4301f`/`b5806695`/`6e1266fe` (sista kodlandningarna) → `eeca8c72` (`#2473`, stängningsbatch 3a, sista landningen). En paus (Marcus till jobbet ~04:35Z), en resume (~14:10Z), en kontrollerad kompaktering (ADR-101, 18:06Z); sessionen avbröts 2026-09-08 utan `session-end` och stängdes administrativt av S125 2026-09-17 (Del 8).
- **Mål:** två prodfynd Marcus observerade själv i eftermiddags-passet — åtgärdssidans plockare visade inte avmarkerade personer, och åtgärdssidans betalningsblock skulle först kopieras från inkorgen, sedan (efter kartläggning) rivas helt — samt Marcus krav på en ny, "sjukt snygg och ren" Öppna detaljer-form på eventdetaljen.
- **Faktiskt:** `TASK-434` plockar-fixen (`#2455` `d62d94f6`) · `TASK-435` rivningen av betalningsblocket (`#2459` `d9bc1c45`) · `TASK-436` Öppna detaljer steg 1, byggd av orkestreraren (`#2457` `01c33c14`, tre granskningsrundor) · `TASK-437` batchad `hamta-inbetalningar` (`#2458` `e4ca9786`, staging + Marcus fas4-deploy v6) · `TASK-438` steg 2, Händelseloggen, byggd av orkestreraren (`#2468` `e4c4301f`) · `TASK-439` tidslinjens ikoner bakom menybaren (`#2467` `b5806695`) · `TASK-309.48` dokumentlistans rullningsskugga (`#2469` `6e1266fe`). Alla sju genom review-loopen; Vercel prod följer `main` för samtliga.
- **Avvikelser:** `TASK-442` (förvärmning av Öppna detaljer, Opus-bygg-agent, `#2474`) nådde runda 2 (exit 20, risk medel — runda 1:s premiss "flaggan är AV i prod" visade sig falsk) när sessionen avbröts utan `session-end`; landar under S125. `TASK-438` landade i kod och fick Marcus ögonmätning men fick aldrig sin Done-flipp (flaggat i S124 Del 8; utanför detta stängningsuppdrags scope att åtgärda).
- **Verifiering:** varje kod-PR genom review-agent i färsk kontext med loop-beslut, sektion och backstopp-preflight (`#2455`/`#2458`/`#2459` konvergerade runda 1; `#2457` tre rundor, mandat-armerad runda 3; `#2467`/`#2468`/`#2469` konvergerade runda 2); Marcus prod-deploy av `hamta-inbetalningar` (v6, `2026-09-08T04:31:49Z`) verifierad via `--kontrollera`.
- **Teknisk skuld / Marcus-moment:** `TASK-442` runda 3 + ögonmätning AC #6 (S125) · `TASK-438` Done-flipp (flaggat, ej åtgärdat) · `TASK-440` (persondetaljens Händelser) · `TASK-441` (serversidigt aktivitetslogg-integritetstest) · `TASK-443` (Lotta-inbjudan, väntar grillning) · hub-lyft av tre `[UNIVERSAL]`-fragment.
- **Full narrativ:** `tasks/sessions/2026-09-08-session-124.md` Del 1–8 + Paushistorik; tolv lessons-fragment i `tasks/lessons.d/` (tre `[UNIVERSAL]`); avslutad administrativt av `tasks/sessions/2026-09-17-session-125.md` Del 1 § Scope, tolkning.

## Session 125 (2026-09-17) — Hemkomsten: nightly grön igen, S124 i mål, dependabot-högen; AFK-stängning på Marcus mandat

- **Commit-range:** `eeca8c72` (S124-stängning, prod vid start) → `87aa3dc4` (`#2491`, kö-blockerarna) → `8d71f77d` (`#2495`/`#2474`/`#2498`) → S125:s docs-batchar (`#2503` + batch 2).
- **Mål:** Marcus tillbaka efter en veckas semester: hur mår repot, hur har nightlyn sett ut, scanna öppna sessioner, triagera. Kvitterat scope: nightly grön igen, S124 i mål, dependabot-högen; därefter *"rent och snyggt, inga issues kvar, ordentliga rotorsaksfixar"*.
- **Faktiskt:** nightly röd 12/12 nätter av sex orsaker — alla åtgärdade: två high-advisories (`TASK-444`, overrides), en tidsinställd landmina i `hem.acceptance` (`TASK-448`, fixturtoken `FROZEN_NOW+24h` gick ut 2026-09-16T08:00Z, `clock.install()` utan `time`) och dess fyra kopior (`TASK-449`), S110 arkiverad (ADR-099-fönstret), Backlog-stängningsgrinden 30 → 0 (tvåfaktors-formen, 241.5/284.4 väg C), 16 `ci-post-merge`-larm stängda med rotorsak (sentinel-driften L599 m.fl.). Dependabot-högen: `TASK-445` (exakt-pin av `@internationalized/date`), `TASK-447` (ADR-028-pinnen luckrad, K0åi fullbordad, risk hög med Marcus GO), `#2481`/`#2484` ersatta, Dependabot stängde resten självt. `TASK-442` runda 3 + `TASK-446` prod-premiss-svepet. S118 steg 4 (`#2298` stängt), S124 stängd administrativt (`#2493`, `TASK-438` Done). Hub-lyft K124.1–K125.4 (`marcus-system#19`).
- **Avvikelser:** de två blockerarna låste varandra → bunt-PR `#2491`; att få väntande PR:er gröna kostade två felvarv (rerun, reopen) innan `update-branch` + omstämpling av fyra kod-PR:er; `#2494` sparkades ur kön när basen landade; zsh-ordsplittning fällde tre loopar; `#2497` flaggades CONFLICTING av GitHub trots ren lokal merge → merge-commit pushad som ny head + andra omstämpling. Nightly-agentens gissning "ny commit" falsifierad mot `git log` (`main` orörd sedan 09-08).
- **Verifiering:** varje kod-PR genom review-agent i färsk kontext, loop-beslut, sektion, backstopp-preflight; omstämplingar med mekanisk diff-identitet; CI på `#2491`:s nya head grönt på audit + alla tre acceptance-shards före kö; manuell nightly-dispatch efter sista landningen (run-id i sessionsdokets § Slutläge).
- **Teknisk skuld / Marcus-moment:** `TASK-442` AC #6 ögonmätning · 241.5/284.4 facit-granskning · 346.9 `--kontrollera` · fyra worktrees med opushat S121/S124-arbete · `TASK-239` (tre gröna nätter) · 346.7.1/346.7 · kortkandidat lokalt-röda acceptance-tester · `funktionsflaggor.ts` § VAD FLAGGAN ÄR TILL FÖR.
- **Full narrativ:** `tasks/sessions/2026-09-17-session-125.md` Del 1–3; nio lessons-fragment i `tasks/lessons.d/` (fyra `[UNIVERSAL]`); S126 parallell (CI-djupgranskning, egen worktree).

---

## Session 127 (2026-09-18 → 2026-09-19) — Kallstarten, bilagor i bulk, betalningsytans korthöjd/ordval; mallväljaren skjuten till ny session

- **Commit-range:** `484ca305` (`#2516`, dokets födelse) → `7ba4ef0d` (Del 11-landningen) → S127:s stängningsbatch (denna PR).
- **Mål:** Marcus tre fynd ur egen prod-användning: (1) en kallstarts-bugg — laddningsskärmens bar stod still och Hem släppte in innan datan var klar, (2) inga bilagor kunde bifogas i Hem-vyns bulkåtgärder ("ett stort problem"), (3) en idé om att bygga om utskicks-block till en mallväljare + mall-editor à la Resend. Punkt 1 och 2 kvitterades till bygge samma session; punkt 3 grillas i en egen, senare session.
- **Faktiskt — kallstarten (`TASK-451`-familjen):** `451.2` räknaren skiljer nu lyckade/misslyckade hämtningar och ny Sentry-tagg `delvis-fel` (`c6983588`). `451.3` Hem delar startvärmningens i-flykt-hämtning i stället för att dubbelanropa (`ab6b2127`). `451.5` preconnect mot Supabase-origin i både prod- och staging-bygget (`b6872fb9`). `451.6` mätpass mot staging: batch 1:s latens är till stor del arkitektur (sekventiell chunk-hämtning i `get-events`), inte bara Airtables 5 req/s-vägg (`815fa3ad`; mintade `458`/`459`). `451.1` skeleton-CLS-delsteg (`fba38b66`) och `451.7` Hem-skeleton (`ad3023f1`) landade som delsteg — full CLS-täckning för Hem, `451.8` (Marcus QA i prod) och ögonmätningarna (`451.1` AC #5, `451.7` AC #4) kvarstår. `451.4` (fyra granskningsrundor, `c5d3882d`): `HAMTNINGENS_TIDSGRANS_MS=160000` på ett ställe, EN retry-policy ersatte 18 tecken-identiska kopior, `TidsgransFel` slutgiltigt i varje lager — Marcus-mandat väg A breddade fixet till hela appen (global `retry: 3` i `router.ts` + ~17 inline-retryfunktioner konsoliderade).
- **Faktiskt — bilagor i bulk:** `452` (fynd, `cdd1856b`) fixade att en gemensam bilaga på ett icke-ursprungsevent gav 400/500 vid sändning — samma delade räckvidds-matchning som `get-event-attachments`, plus staging- och prod-deploy av båda EF:erna (prod-deploy utfört av Marcus 2026-09-19 08:57–09:02Z). `455` (`018dfeef`) gav Hem-vyns svep en bilageväljare per eventgrupp (`Bekräfta alla`/`Påminnelse till alla`/`Eventinfo`) — konvergens mot det låsta facitet `s102-svep-konvergens`, stämplad av Marcus i dev-server; en efterföljande stämpel-omprövning (utökad form) fälldes av `check-facit.sh`s rivningsklausul och backades av Marcus själv (tråd `T188`).
- **Faktiskt — betalningsytan:** `456` (tre rundor, `168dd403`) reserverade pill-radens höjd så alla inkorgskort blir exakt lika höga — rundorna prövade tre olika former (`min-h`, kompakt etikett, till sist beskedet flyttat ut ur pill-raden till beloppsraden). `475` (`0cbd12f6`) bytte "Basen släpar" mot en synlig, fullständig mening ("Databasen har inte hunnit uppdateras för N betalningar. Beloppen här i appen stämmer.") efter att Marcus inte förstod den gamla, dolda `title`-texten. `481` (`3f3b1aed`) fixade en regression `456` orsakade: skeleton-kortet saknade pill-radens höjdreservation, vilket höll Post-merge röd på fyra kod-landningar i rad tills det fångades och lagades med staging-bevis.
- **Avvikelser, öppet bokförda:** kvotavbrott (HTTP 429) dödade tre bygg-agenter mitt i arbetet — worktrees lästes före uppväckning, allt ocommittat arbete bevarades. En kontrollerad kompaktering (ADR-101) kördes en gång. Tre nya mätta celler i worktree-isoleringens gräns (`EnterWorktree` mot en annan befintlig worktree, en stoppad agents `SendMessage`-återupptagning, `!`-prefixets isolering) — se `CLAUDE.md` § Worktree-isoleringens gräns. Orkestreraren förde två gånger vidare ett oprövat påstående (eget: "Basen släpar" togs som given trots flytt; ärvt: en granskares claim om `fetchPersonsRegister` visade sig falsk). En föreslagen prod-EF-deploy för `459`s loggning STRUKEN ur sessionen på Marcus uttryckliga order ("Du gör det för avancerat för mig").
- **Verifiering:** varje kod-PR genom review-agent i färsk kontext (risknivåer låg→hög, upp till fyra granskningsrundor på `451.4`), loop-beslut, Riskbedömnings-sektion, backstopp-preflight före armering. `481` bevisad live mot staging (19/19, `test:e2e:staging` normal väg). `452` bevisad staging→prod (400→500→200-kedjan, UPDATED_AT-verifiering). `455` bevisad staging (bilage-bärande sändning + negativ kontroll).
- **Uppskjutet / kvar:** prod-EF-deploy av `459`s loggning (struken, ej nödvändig för driften) · 429-mätningen i staging som ska avgöra `458` (draft `#2550`) · trådar `T186`–`T188` · kort `463`, `476`–`478` · Marcus QA `451.8` + ögonmätningar. **Mallväljaren och mall-editorn (Marcus punkt 3) är INTE rörda i S127** — Marcus beslut: en ny, egen session, som börjar med hans `/grill-me` innan något annat görs.
- **Full narrativ:** `tasks/sessions/2026-09-18-session-127.md` Del 1–12; 20 lessons-fragment i `tasks/lessons.d/` (S127:s stängningsbatch, flertalet `[UNIVERSAL]`).
