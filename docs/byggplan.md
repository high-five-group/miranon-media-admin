...
| **6.5** | 🟡 PÅGÅR | Aktivitetslogg (xAPI). `requestId`-mönstret från Fas A M7 ärvs. **Byggd och LIVE i prod sedan 2026-08-13 ~18:30** (`tasks/sessions/2026-08-11-session-105.md` § Del 8 + Paushistorik; `activity_log`/`log-activity`/`get-activity-log` ACTIVE, 15/15 mutationshooks loggar) — QA (`task-201.10`) + stängning återstår, ✅ KLAR sätts vid fas-avslutets phase-end-verify, inte här. | 1 session |
