...
test.describe('Primitiver — axe-core 0 violations (ADR-045)', () => {
  ...
  test('InstallPrompt — sektion (chromium-prompt fångad: knappen synlig)', async ({
    ...
    // Skjuter UPPREPAT (samma robusta mönster som acceptance-sviten,
    // `install-prompt.acceptance.test.ts`) tills knappen syns — ett enda
    // synkront dispatch-anrop kan racea mot Reacts useEffect-mount.
