...
// [TASK-243.1] Följande sju tester i denna describe skippas individuellt —
// ÖPPET bokförd, minimal anpassning per kortets DoD-gräns ("gör MINIMAL
// anpassning så CI är grönt per jobb — den FULLA omskrivningen är skiva
// 243.3:s jobb"). ADR-102/103-promoveringen (V1 "Lugna morgonen") ersatte
// K10-formens block (Obetalda-kortet, CTA:n "Visa alla anmälningar",
// helkorts-klick på Nästa event, region "Nya anmälningar att hantera") med
// Morgonkollens nya block (se `Hem.tsx`). De två resterande testerna i denna
// describe (get-registrations 4xx, axe 0) testar beteende som är oförändrat
// och lämnas RÖRDA.
test.describe('Hem — A-skelettet (task-1.3)', () => {
  test('A-skelettet renderas: hälsnings-h1, kort med data, CTA', async ({ page, network }) => {
    test.skip(true, '[TASK-243.1] CTA/Obetalda-kortet retirerade — se task-243.3');
    mock(network, {
      registrations: [
  ...
  test('AC 2 — klick var som helst på Nästa event-kortet → eventets detaljsida', async ({
    ...
    // [TASK-243.1] Helkorts-klick (stretched link) är INTE promoverat — V1-
    // facitet länkar bara eventnamnet (`NastaEvent.tsx`), byte-identiskt med
    // prototypens `VariantRo.tsx`. Se task-243.3.
    test.skip(true, '[TASK-243.1] Helkorts-klick retirerat — se task-243.3');
    mock(network, {
      registrations: [reg()],
  ...
  test('task-4.4 AC 2 — klick på anmälningsrad → EVENTETS sida (B1)', async ({ page, network }) => {
    // [TASK-243.1] "Nya anmälningar" filtrerar nu på status===Obekräftad
    // (räknar-rubrikens facit-krav, `hem-derivations.ts` obekraftadeAnmalningar)
    // — det gamla kortet visade ALLA registreringar recency-baserat utan
    // statusfilter. Fixturens Carl (status "Bekräftad") försvinner därför ur
    // listan; testet timar ut. Se task-243.3.
    test.skip(true, '[TASK-243.1] Statusfiltret ändrat (facit-krav) — se task-243.3');
  ...
  test('task-1.4 AC 2 — Hem-CTA:n landar på samlade anmälningslistan', async ({
    ...
    // [TASK-243.1] `CTA.tsx` ("Visa alla anmälningar") är RETIRERAD — ersatt
    // av Genvägar-blocket (`Genvagar.tsx`, /anmalan/ny + /atgarder). Se
    // task-243.3.
    test.skip(true, '[TASK-243.1] CTA.tsx retirerad — se task-243.3');
  ...
  test('task-4.4 AC 2 — rad utan event-koppling är olänkad och visar "Utan event"', async ({
    ...
    // [TASK-243.1] Samma statusfilter-skäl som föregående test + region
    // "Nya anmälningar att hantera" retirerad. Se task-243.3.
    test.skip(true, '[TASK-243.1] Statusfiltret ändrat + region retirerad — se task-243.3');
    mock(network, {
  ...
  test('tomma listor → vänliga tom-texter per card, inga fel', async ({ page, network }) => {
    // [TASK-243.1] Tom-texterna + regionnamnen ("Obetalda anmälningsavgifter")
    // är retirerade — nya formens tom-texter: `NyaAnmalningar.tsx`/
    // `ForfallnaBetalningar.tsx`. Se task-243.3.
    test.skip(true, '[TASK-243.1] Tom-texter/regioner ändrade — se task-243.3');
  ...
  test('endast dåtida event → "Inga kommande event"', async ({ page, network }) => {
    // [TASK-243.1] Exakt textform ändrad: "Inga kommande event just nu."
    // (`NastaEvent.tsx`, byte-identisk med facit-prototypen). Se task-243.3.
    test.skip(true, '[TASK-243.1] Tom-eventtexten ändrad (facit-form) — se task-243.3');
    mock(network, {
      registrations: [reg()],
...
test.describe('Hälsningen (task-1.1 — namnkällan ur kontots metadata)', () => {
  ...
  test('TASK-220 AC 1 — fullt display-namn → hälsningen visar ENDAST förnamnet, båda lägena', async ({
    ...
    // [TASK-243.1] B2 (sessionStorage-flaggan "bara namnet vid återbesök")
    // är INTE promoverad — ÖPPET bokförd avsiktlig avvikelse i `data.ts`s
    // docblock (prototypens `fornamn`-härledning: "Hej {förnamn}" visas
    // ALLTID, aldrig det avskalade "{förnamn}"). Förnamns-extraktionen
    // (huvudpåståendet i testnamnet) håller fortfarande — bara
    // återbesöks-halvan faller. Se task-243.3.
    test.skip(true, '[TASK-243.1] B2-återbesöksformen ej promoverad — se task-243.3');
 ...
 */
// [TASK-243.1] AC1/AC2+3/AC5 skippas individuellt (ÖPPET bokförd, minimal
// anpassning) — dagar-kvar-PILLEN (positionerad topp-höger) är INTE
// promoverad (V1-facitet visar formen som en vanlig rad under rubriken,
// `NastaEvent.tsx`, byte-identisk med `VariantRo.tsx`), och "Obetalda
// anmälningsavgifter"-kortet är retirerat (ersatt av "Förfallna
// betalningar", `ForfallnaBetalningar.tsx`). AC4 (beläggningsstapeln) är
// OFÖRÄNDRAD och lämnas RÖRD. Se task-243.3.
test.describe('Nästa event + Obetalda till facit (task-4.3)', () => {
  test('AC 1 — dagar-kvar-pillen: tre exakta former, vit pill topp-höger', async ({
    ...
    test.skip(true, '[TASK-243.1] Dagar-kvar-pillen ej promoverad — se task-243.3');
  ...
  test('AC 2+3 — metagrupp med ikoner + långdatum; EN länk-yta; kortrubriken text-xl semibold mörk RENDERAT', async ({
    ...
    // [TASK-243.1] Kortrubrikens typografi ändrad: V1-facitet bär en liten
    // versal etikett ("NÄSTA EVENT", caption/medium/secondary) i stället för
    // K10:s text-xl/semibold/mörk rubrik (`NastaEvent.tsx`). Se task-243.3.
    test.skip(true, '[TASK-243.1] Kortrubrikens typografi ändrad (facit-form) — se task-243.3');
    mock(network, {
      registrations: [],
      events: [
        ev({
  ...
  test('AC 5 — Obetalda anmälningsavgifter visar ENDAST antalet, text-3xl semibold', async ({
    ...
    // [TASK-243.1] `ObetaldaCard.tsx` är retirerad — ersatt av "Förfallna
    // betalningar" (start−14d-deadline-definitionen, `ForfallnaBetalningar.tsx`).
    // Se task-243.3.
    test.skip(true, '[TASK-243.1] ObetaldaCard.tsx retirerad — se task-243.3');
    mock(network, {
      registrations: [
...
test.describe('Hem-strukturen till K10-facit (task-4.2)', () => {
  test('hälsningen utan utropstecken; återbesök i sessionen visar bara namnet (B2)', async ({
    ...
    // [TASK-243.1] B2 (återbesök visar bara namnet) är INTE promoverad — se
    // samma rationale som TASK-220 AC1-testet ovan. Utropstecken-frånvaron
    // (huvudpåståendet) håller fortfarande. Se task-243.3.
    test.skip(true, '[TASK-243.1] B2-återbesöksformen ej promoverad — se task-243.3');
    mock(network);
    await page.goto('/hem');
  ...
  test('versionsraden nere till vänster endast desktop och matchar paketmanifestet (AC 5)', async ({
    ...
    // [TASK-243.1] Versionsraden fanns INTE i facit-prototypen och
    // promoveras därför inte (ÖPPET bokförd avvikelse, se `Hem.tsx`s
    // docblock + slutrapporten för TASK-243.1 — ADR-102 B1 väger tyngre än
    // att tyst återuppfinna en yta facit inte visar). Se task-243.3.
    test.skip(true, '[TASK-243.1] Versionsraden ej promoverad (facit saknar den) — se task-243.3');
 ...
 *
 * [TASK-243.1] HELA DESCRIBEN skippas — ÖPPET bokförd, minimal anpassning
 * (kortets DoD-gräns). Samtliga sex tester mäter K10-formens specifika
 * kort-chrome (koppar-kontur + koppar-ikon, zebra-radning, region "Nya
 * anmälningar att hantera") som ADR-102/103-promoveringen (V1 "Lugna
 * morgonen") ersatte med `NyaAnmalningar.tsx`s form. Full omskrivning:
 * task-243.3.
 */
test.describe
  .skip('Anmälningslistan till facit (task-4.4)', () => {
    ...
    test('AC 3 — zebra varannan rad utan skiljelinjer, rundade rader (renderat)', async ({
      ...
      for (const [i, stil] of stilar.entries()) {
        expect(stil.border, `rad ${i} ska sakna skiljelinjer`).toEqual([
          '0px',
          '0px',
          '0px',
          '0px',
 ...
 *
 * [TASK-243.1] HELA DESCRIBEN skippas — ÖPPET bokförd, minimal anpassning
 * (kortets DoD-gräns). Fixturernas registreringar saknar status "Obekräftad"
 * (default i denna fils `reg()`), och `NyaAnmalningar.tsx` filtrerar nu på
 * just den statusen (facit-krav, `hem-derivations.ts` obekraftadeAnmalningar)
 * — raderna testet letar efter (`getByRole('link', { name: /Anna
 * Andersson/ })` m.fl.) renderas därför aldrig. Poll-lagrets osynliga
 * uppdatering i sig är OFÖRÄNDRAD (samma `useDashboardData.ts`-hooks) — bara
 * fixturerna och DOM-strukturen denna fil mäter mot är föråldrade. Full
 * omskrivning: task-243.3.
 */
test.describe
  .skip('Osynliga uppdateringen (task-4.5)', () => {
    ...
    function hallbarMock(
      ...
      const svara = (arEvents: boolean) =>
        arEvents
          ? json({ events: st.data.events })
          : json({ registrations: st.data.registrations });
