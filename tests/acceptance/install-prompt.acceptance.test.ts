...
/**
 * InstallPrompt — externt beteende (task-126.2; PRD task-126 §Testbeslut:
 * "Install-ytans externa beteende ... testas i acceptance-skarven —
 * hermetiskt, utan staging").
 *
 * Kör mot `/dev/primitives` (ADR-044-demoytan, dev-guardad men nåbar i
 * acceptance-projektets `npm run dev`-server) — konsumentytan (Mer-fliken)
 ...
 * a11y-projektets auto-navigate-fixture ger det gratis. Acceptance-projektet
 * saknar den fixturen (sidan är `about:blank` tills testet gör sitt EGET
 * första `goto`), så `satt()` här avslutar med `page.goto()`, aldrig
 * `reload()`, för just den första navigeringen.
 *
 ...
 * navigerar dit (acceptance-projektet saknar a11y-projektets auto-navigate-
 * fixture — sidan är `about:blank` tills testet själv gör sitt FÖRSTA
 * `goto`; ett `reload()` här reloadar `about:blank`, inte demoytan — det var
 * den ursprungliga bugg som gjorde alla plattforms-tester röda av fel skäl).
 */
async function satt(
  page: Page,
  opts: {
