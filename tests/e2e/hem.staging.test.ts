 ...
 * temporalt, obetalda-antal), greeting, CTA→/event, fokus→<h1>, tom-state per card,
 * fel (4xx role=alert, no-retry), axe 0.
 */
...
test.describe('Hem-aggregering (Fas 6d L1 — statisk översiktsvy)', () => {
  test('cards renderas med data + greeting + CTA; fokus → <h1>', async ({ page }) => {
    await mock(page, {
      registrations: [
    ...
    // <h1> = "Hem", fokuserad efter montering.
    const heading = page.getByRole('heading', { level: 1, name: 'Hem' });
