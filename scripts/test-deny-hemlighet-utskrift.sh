...
# TASK-185 (2026-08-26, körning 32932454040/32933091898): en-katalogs-
# borttagningen (nedan, ursprunglig form) döljer bara den FÖRSTA träffen
# `command -v jq` råkar resolva till — otillräckligt på ubuntu-latest, som
# rutinmässigt exponerar jq via mer än en PATH-katalog. Att STRYKA hela
# katalogen (en tidigare, redan förkastad fix i scripts/test-deny-facit-
# godkand-skrivning.sh) är ANNU farligare på merged-usr-plattformar
# (`/bin` är symlänk till `/usr/bin`, samma fysiska katalog som TVÅ
# PATH-segment): den katalogen håller inte bara jq utan ALLA coreutils —
# att stryka den tar bort `dirname`/`bash` också och kraschar hela
# körningen (exit 127). Samma rotorsak, se test-deny-facit-godkand-
# skrivning.sh:s path_utan() § FIXEN för fullständig historik + kontrollerad
# repro. Fixen här: shimma en FILTRERAD symlänk-kopia (allt UTOM jq) i
# stället för att stryka hela segmentet.
PATH_NO_JQ=""
compute_path_no_jq() {
