...
// Pillens states via React Arias data-attribut (data-selected/data-disabled)
// — inte :hover/:active — så pekare, tangentbord och touch får identisk
// semantik. Vald pill lyfter i bg-token + semibold + skugga (facitets
// vita pill); ovald är tyst i sekundärfärgen. Statisk (ingen transition) —
// reduced-motion/print utan specialfall (NavCard-precedenten). Fokusringen
// kommer från den globala *:focus-visible-regeln i base.css.
const itemVariants = cva(
  'select-none rounded-full text-center font-medium text-text-secondary data-[selected]:bg-bg data-[selected]:font-semibold data-[selected]:text-text data-[selected]:shadow-sm data-[disabled]:cursor-not-allowed data-[disabled]:opacity-50',
  {
    variants: {
      size: {
 ...
 * `ToggleButton` med `aria-hidden`-ikon som innehåll.
 *
 * @example
 * ```tsx
 * <ToggleButtonGroup
