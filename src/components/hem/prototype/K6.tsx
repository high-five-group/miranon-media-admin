...
/**
 * [PROTOTYPE] K6 — Marcus K5-feedback 2026-07-07 (S55 Del 7):
 * kortrubrikerna FLYTTADE UT ur korten (sätts ÖVER den tonala ytan) och
 * färgen TILLBAKA till ordinarie dämpade grå (neutral-500 — ljusgrå-testet
 * förkastat); menyn TILLBAKA till K1-formen (verkliga botten-tabbaren på
 * alla breakpoints — vertikala varianten förkastad); anmälningsraderna i
 * historikens teckenstorlek (text-caption) + ZEBRA-test (mörkare grå
 * varannan rad, avdelar-linjerna borta); aktivitetsloggen med svag
 * fyllton (bg-subtle — "liten brytning", inte vit, ingen kant);
 * koppar-konturen på anmälningskortet KVAR (varvets godkända element).
 * Inline-style på rubrikerna krävs alltjämt (base.css olagrade
 * h1–h6-regeln, Del 6-byggkravet). Kastas med prototypen.
 */
...
function DashboardCardK6({
  ...
  return (
    <section aria-labelledby={headingId} className="flex flex-col gap-1.5">
      {/* Rubriken utanför den tonala ytan; inline-style pga base.css-regeln (Del 6). */}
      <h2 id={headingId} className="text-small" style={{ color: 'var(--p-neutral-500)' }}>
        {title}
      </h2>
      <div
        className={`relative flex flex-1 break-inside-avoid flex-col gap-1 rounded-2xl border border-transparent p-4 contrast-more:border-border-strong print:border-border-strong ${
...
function NyaAnmalningarCardK6() {
  ...
  return (
    <section aria-labelledby={headingId} className="flex flex-col gap-1.5">
      ...
      {/* Koppar-konturen — varvets godkända element. */}
      <div className="relative flex break-inside-avoid flex-col gap-1 rounded-2xl border border-[color:var(--mm-accent)] bg-bg-muted p-4 print:border-border-strong">
          ...
          {ANMALNINGAR_EXEMPEL.map((rad, i) => (
            ...
            >
              <Link to="/event" className="group flex flex-col gap-0.5 px-2 py-2">
                <span className="truncate font-semibold text-caption group-hover:underline">
                  {rad.namn}
                </span>
                <span className="truncate text-caption">{rad.event}</span>
...
// ── K6 — komposition: K1-menyn tillbaka (botten-tabbaren, alla breakpoints) ──
export function K6() {
  return (
    <div className="relative min-h-dvh">
      <div className="mx-auto flex w-full max-w-[640px] flex-col gap-3 p-4 pt-6 pb-24 lg:pt-14">
