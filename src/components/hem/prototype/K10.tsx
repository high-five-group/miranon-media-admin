...
/**
 * [PROTOTYPE] K10 — Marcus K9-feedback 2026-07-07 (S55 Del 11):
 * omhämtningen HELT OSYNLIG (Marcus reviderar dumpens blur-formulering):
 * stale-while-revalidate — previous data renderas orörd under tyst
 * bakgrundshämtning, skärmen ändras ENDAST där värden faktiskt ändrats;
 * all visuell fetch-indikation (blur/gråning) BORTTAGEN.
 * `data-proto-fetching` behålls OSYNLIGT som verifierings-krok.
 * Första kalla inladdningen behåller laddläget (inget att visa före
 * första datat — ärligt undantag; persist-cache är en senare förfining).
 * Demo-knappen kvar: klicket ska nu INTE synas. Övrigt oförändrat från
 * K9. Kastas med prototypen.
 */

// ── K10-lokala data-hooks: previous data behålls under omhämtning ─────────────
...
function useEventsK10() {
  ...
  return useQuery({
    ...
    ...DASHBOARD_POLLING_K10,
  });
}

function useRegistrationsK10() {
  const dataSource = useDataSource();
...
function DashboardCardK10({
  ...
  return (
    ...
    >
      ...
      {/* Osynlig bakgrundsuppdatering (stale-while-revalidate): previous
          data renderas orörd; ingen visuell fetch-indikation. */}
      <div data-proto-fetching={isFetching}>
        {isPending ? (
          <p
            role="status"
            aria-live="polite"
...
function NastaEventCardK10() {
  const { data, isPending, isFetching, isError, error } = useEventsK10();
...
function ObetaldaCardK10() {
  const { data, isPending, isFetching, isError, error } = useRegistrationsK10();
...
function NyaAnmalningarCardK10() {
  const headingId = useId();
  ...
  return (
    ...
    >
      ...
      <div data-proto-fetching={fetching} className="flex flex-col gap-1">
...
// ── K10 — komposition: K1-menyn tillbaka (botten-tabbaren, alla breakpoints) ──
export function K10() {
  return (
