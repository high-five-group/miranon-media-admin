...
/**
 * [PROTOTYPE] K5 — Marcus K4-feedback 2026-07-07 åtgärdad (S55 Del 6):
 * kortrubrikerna i FK-FORMEN (IMG_1538: sentence case, ljusgrå — INTE
 * versal/spärrad/koppar; neutral-400 #898989, renderad färg ASSERTAD i
 * browsern — K4:s kopparklass genererades aldrig, rotorsaken till "ingen
 * skillnad"); menyn med K1-barens EXAKTA mått flippade (60×568, celler
 * ~52×136 = barens cellmått) topplinjerad med innehållet; innehållet
 * nedflyttat (ingen menyrad upptill längre); "Miranon Media Admin v0.1.0"
 * (appnamnet tillbaka framför versionen); anmälningsrader med relativ tid
 * ("för 2 tim sedan"-formen) UTAN chevron; aktivitetsloggen chromeless på
 * vit bakgrund utan punkt; anmälningskortet med koppar-kontur (fokus-test).
 * FK-linjen (docs/reference/fk-referens/) är designramen. Kastas med
 * prototypen.
 */
...
function VertikalTabbarK5() {
  return (
    <nav
      aria-label="Huvudnavigation"
      className="fixed top-14 right-[calc(50%+340px)] hidden h-[568px] w-[60px] rounded-full border border-border bg-surface contrast-more:border-border-strong lg:block"
    >
...
function CardEtikett({ id, children }: { id: string; children: ReactNode }) {
  return (
    // Inline-style krävs: base.css h1-h6-regeln är OLAGRAD och slår
    // utilities-lagret (K3/K4-rotorsaken till "ingen färgskillnad";
    // byggkrav: flytta regeln till @layer base).
    <h2 id={id} className="text-small" style={{ color: 'var(--p-neutral-400)' }}>
      {children}
    </h2>
  );
}

function DashboardCardK5({
  title,
  tone = 'neutral',
  isPending,
  isError,
...
function NyaAnmalningarCardK5() {
  const headingId = useId();
  return (
    <section
      aria-labelledby={headingId}
      className="relative flex break-inside-avoid flex-col gap-1 rounded-2xl border border-[color:var(--mm-accent)] bg-bg-muted p-4 print:border-border-strong"
    >
          ...
          >
            <Link to="/event" className="group flex items-center justify-between gap-3 py-3">
              <span className="flex min-w-0 flex-col gap-0.5">
                ...
                <span className="text-small text-text-muted">{rad.anmald}</span>
              </span>
            </Link>
          </li>
        ))}
...
function AktivitetK5() {
  const headingId = useId();
  return (
    <aside
      aria-labelledby={headingId}
      className="absolute bottom-0 left-full ml-5 hidden w-72 flex-col gap-1 rounded-xl bg-surface p-3 xl:flex"
    >
      <h2 id={headingId} className="text-small" style={{ color: 'var(--p-neutral-400)' }}>
        Senaste aktivitet
      </h2>
...
// ── K5 — komposition: kolumnen ALLTID skärm-centrerad ────────────────────────
export function K5() {
  return (
    <div className="relative min-h-dvh">
      <VertikalTabbarK5 />

      <div className="mx-auto flex w-full max-w-[640px] flex-col gap-3 p-4 pt-6 pb-24 lg:pt-14 lg:pb-8">
