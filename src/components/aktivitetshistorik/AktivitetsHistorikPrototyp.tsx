...
/**
 * [PROTOTYPE] S106 konvergens-pass — KASTBART SUBSTRAT tills promovering
 * (promoveringskontraktet ADR-103: den godkända FORMEN promoveras, det som
 * rivs är växlar och villkor).
 *
 * FRÅGAN: "Hur ska aktivitetshistorik-sidan se ut — omdesignad från nuläget?"
 * Konvergensfas DIREKT på befintlig yta; divergens-passet (tre varianter)
 * öppet bortvalt på Marcus order 2026-08-15 ("vi kan utgå från nuläget och
 * itererar på den direkt"). Steg 1 = EXAKT kopia av skarpa
 * `AktivitetsHistorik.tsx` (UI.md § Konvergens-passet — kopia av FAKTISKA
 * vyn, inte av någon variant). Datavägen ärvs oförändrad (read-only läsvy
 * via `useActivityLogHistory` → get-activity-log-EF; inga mutationer).
 *
 * RIVNING (efter Marcus `godkand` i facit-manifestet, aldrig före —
 * `scripts/check-facit.sh` grindar): route-wiringen i
 * `src/routes/_authenticated/mer/aktivitetshistorik.tsx` återställs och
 * denna fil `git rm`:s när formen promoverats in i skarpa filen.
 */
 ...
 */
export function AktivitetsHistorikPrototyp() {
