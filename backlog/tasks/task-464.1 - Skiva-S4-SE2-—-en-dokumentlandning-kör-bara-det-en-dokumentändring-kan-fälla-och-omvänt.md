...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Kontrastpar, mätt med gh run view --json jobs: en D0-klassad PR startar ingen runner för typkontroll eller Biome; en kod-PR kör båda oförändrat
- [x] #2 Per flyttad dokumentgrind finns ett skrivet svar i PR-kroppen på frågan 'kan en ren kodändring fälla den?' — varje grind med svaret ja står kvar på kodändringar
- [x] #3 Aggregatorn ci-passed förblir fail-closed: skipped godkänns ENDAST för de jobb som villkorats här; scripts/check-aggregator-needs.mjs, scripts/check-listparitet.sh och gate-proof gröna
- [x] #4 npm run verify:ci-parity:fast grön; .ci-parity-policy.json och .listparitet-policy.conf följer med; CLAUDE.md-prosan om lintjobbet rättas endast där den blivit falsk, med mätta tal
- [x] #5 PR-kroppens sektion 'Kostnad i två mått': fakturerade minuter för en ren markdown-landning före (35, #2517) och efter, mätt per yta på en verklig körning, plus väntetid före/efter och månadseffekt vid 1 279 landningar
- [x] #6 Inget verkligt skydd borta: säkerhetsskanning, tillgänglighets- och hermetikjobb samt review-backstoppen kör exakt som förut på kodändringar — bevisat med jobblistan från kod-PR:en i kontrastparet
<!-- AC:END -->
...
Landad som #2564 (`c5fdc75c`, 2026-09-19 09:50Z; head `38e800b7`, tre granskningsrundor — runda 3: risk medel, 0 error; armerad på Marcus beslut efter exit 20). S4 + SE2: kodgrindarna (Lint + TypeCheck, testsvit, beroendegranskning, review-backstopp) hoppas på en ren D0-diff via en POSITIV, fail-closed lista över belagt lint-inerta ändelser (`.lint-inert-policy.conf`: md jsonl txt pdf png jpg jpeg gif webp csv; svg struken — Biome parsar den), och dokumentgrindarna körs bakom dokumentvillkoret. MÄTT på första D0-landningen efter merge (#2572): väntan per runda 4,1 + 4,3 → 1,5 + 1,3 min; fakturerat 31 → 13 min per dokumentlandning; ingen CodeQL-körning; kontrastparet i kortets notes. EFTERKONTROLLEN på `c5fdc75c` blev RÖD (körning 35435803515: 308 passed, 1 failed, 1 flaky) — på ett test som INTE rörs av denna ändring: `mer-betalningar-laddlage.staging.test.ts:275` föll första gången på `168dd403` (#2541, S127) och ärvs av varje kodlandning därefter; diagnosen står i #2573, vårt larm #2577 stängt med skriven motivering. Uppskjutet med avsikt: två info-fynd (falsk kommentar ci.yml ~1061, stale tal verify-ci-parity.mjs ~833) bokförda på TASK-471. AC #1:s D0-sida kunde bara bevisas EFTER merge — gjort.
