...
## Fix-runda 4 (review runda 4, Marcus-beslut 2026-09-19)

Runda 4 (risk medel; 111/111 och shellcheck 0 bekräftade av granskaren) fann tre fynd, alla avgjorda av orkestreraren:

**Fynd 1 (ERROR) — BYGGT.** `exit 2` för ogiltigt `--session`-ID (fix-runda 3) KOLLIDERADE med skriptets egen bitmask `2 = DIRTY` — en läsare av `$?` kunde inte skilja "ogiltigt ID, inget svep" från "en konfliktad PR". Felet var orkestrerarens eget (dikterade `exit 2` utan att läsa exitkods-konventionen). Rättat till skriptets BEFINTLIGA `die()` med dess DEFAULT `exit 64` — ingen ny mekanism, bara konsekvent användning av det som redan fanns för REPO/INTERVAL/TIMEOUT/okänt argument. `die()` skriver ENDAST till stderr (etablerad konvention) — den dubbla stdout+stderr-utskriften fix-runda 3 byggde för just detta fall är borttagen.

**Fynd 2 (warning, pre-existing men inom anspråket) — BYGGT.** `--session` (eller VILKEN som helst av de fem värde-tagande flaggorna) som SISTA token utan värde fick `shift 2` att fallera — `set -e` avslutade skriptet med `shift`s EGEN exitkod (1 = bitmask RÖTT) och NOLL utskrift. Fångat med `[[ $# -ge 2 ]]`-vakt INNAN `shift 2` i alla FEM grenar (`--repo`/`--branch`/`--interval`/`--timeout`/`--session` — identiskt `"${2:-}"; shift 2`-mönster i alla fem innan denna runda) ⇒ `die` med tydligt meddelande, exit 64.

**Fynd 3 (info) — BYGGT, VALT ALTERNATIV (skiftlägesokänsligt med avsikt).** "Kollision omöjlig per konstruktion" (fix-runda 3) höll på strängnivå men inte på ett skiftlägesokänsligt filsystem — macOS APFS delar fil för "S126"/"s126" oavsett vad valideringen bevisar om bash-strängar. Statsfilens suffix normaliseras nu till GEMENER (portabel `tr '[:upper:]' '[:lower:]'`, INTE `${var,,}` — kräver bash 4+, macOS-default-bash är 3.2, samma begränsning som `is_exempt_author()` redan dokumenterar) INNAN det används i filnamnet. "S126" och "s126" ÄR med avsikt SAMMA session för de tre strypta notiskanalerna sedan denna runda — dokumenterat i både kod-kommentar och `.heartbeat-svep-policy.conf`. VIKTIGT AVGRÄNSAT: markör-matchningen (`pr_har_session_marker()`, RÖTT/DIRTY/KANDIDAT-filtreringen mot bygg-agentens PR-kroppsmarkör) är en ANNAN, HELT ORÖRD mekanism och förblir skiftlägeskänslig — bara statsfilnamnet normaliseras. Testat explicit (T82: en PR märkt "S126" matchas INTE av `--session s126`).

**Bitmask-genomgång (uppdragets explicita krav — "gå igenom ALLA nya felvägar"):** grep:ade samtliga `exit`/`die`-anrop i hela filen. Ingen annan ny felväg kolliderar med bitmask-rymden (0/1/2/4/77, kombinationer 1–7). `session_id_sanitize()`-ersättningen (HEARTBEAT_SESSION_ID_REGEX-konstanterna) och per-session-statsfil-städningen (i `stada_grenar_om_dags()`, fix-runda 3) `return`ar alltid 0, aldrig `die`/`exit`.

**UPPTÄCKT UNDER REVISIONEN, EJ BYGGT (utanför uppdragets uttryckliga scope):** `--session --alla` tolkar `"--alla"` som ett GILTIGT sessions-ID (matchar `HEARTBEAT_SESSION_ID_REGEX` — bindestreck och bokstäver är tillåtna tecken) i stället för att avvisas som "flagga utan eget värde". Detta är INGEN bitmask-kollision — resultatet är en giltig (om än oavsedd) klassificering med exit 0 eller motsvarande vanlig verdikt, inte en felkod som kan förväxlas med ett PR-larm. Uppdraget bad specifikt om "`--session` som SISTA token" (`shift 2`-felet, fynd 2) — detta är en ANNAN, mildare brist (grädig konsumtion av nästa token oavsett om den råkar se ut som en flagga). Byggde INTE en fix för detta (skulle kräva att känna igen "börjar med `--`" som en särskild signal, vilket är en bredare designfråga än denna rundas mandat) — rapporterat öppet i stället för att tyst utöka scope. Ett testfall (T79b) som ursprungligen skrevs för detta togs bort igen eftersom det testade obeställt beteende.

**Testmetod-fynd (eget, under testbygget):** `[[ -f ".../last-omarkerad-notis-S126" ]]` (versal-check) gav ett FALSKT POSITIVT på macOS APFS (skiftlägesOKÄNSLIGT för fil-LOOKUP som default) — testet hittade den redan skapade gemener-filen "...s126" via en case-insensitive lookup, oavsett att den FAKTISKA lagrade filen var gemener. Löst genom att läsa `find -name`s returnerade katalogsträng (skiftlägeskänslig STRÄNG-jämförelse, ingen ny filsystem-lookup) och `grep -F` (case-sensitive) mot den — inte genom en `-f`-existens-check. Dokumenterat i testkommentaren som en vägledning för framtida skiftläges-tester i detta repo.

**Differentialbevis:** 13 av de (om)skrivna testfallen (T64, T68–T72, T75–T79, T81, T81b) fälls DETERMINISTISKT mot den orörda runda-4-koden (git show 8c5d77b69d4ef98d6c23df4fd6475b687181029b:scripts/heartbeat-svep.sh) — 108 passerade / 13 failade i en isolerad scratchpad-kopia — och passerar samtliga mot den fixade koden (121/0).

**Testsvit:** scripts/test-heartbeat-svep.sh 111 → 121 fall (10 nya: T75–T79 saknat-flaggvärde för de fem värde-flaggorna, T80/T80b/T81/T81b skiftlägesokänslig statsfil, T82 markör-matchningen förblir skiftlägeskänslig — T64/T66–T72 omskrivna med ny exit-kod/ström-förväntan, samma namn men annan bevisbörda), 0 failade, exit 0. shellcheck --severity=style --enable=all mot CI:s fulla filsvit: exit 0, 0 diagnoser. bash -n scripts/heartbeat-svep.sh: syntax OK.

**Rörda filer i denna fixrunda:** scripts/heartbeat-svep.sh (exit 2→64 via die(), SESSION_GIVEN-flödets vaktar utökade med [[ $# -ge 2 ]] för alla fem värde-flaggor, SESSION_LOWER/tr-normalisering i SESSION_STATE_SUFFIX, § EXIT-KODER-dokumentationen rättad och utökad, § SESSIONSMEDVETET SVEP-headern med nytt SKIFTLÄGESOKÄNSLIGT-stycke, --help-range 61,286 omräknad, flera "exit 2"-referenser i prosa korrigerade till "exit 64"), scripts/test-heartbeat-svep.sh (T64/T66-T72 exit-kod och ström-förväntan uppdaterad, T75-T82 nya, T79b skriven och sedan borttagen igen som obeställd), .heartbeat-svep-policy.conf (ingen ändring denna runda — fynd 1/2 rör bara skriptets egen argumentparsning, fynd 3:s normalisering behövde ingen ny config-rad). Riskbedömnings-sektionen i PR-kroppen rördes INTE (uppdragets regel, verifierat byte-identisk pre/post via diff mot den levande PR-kroppen).
