...
//
// VARFÖR STANDARD/KOPIA REDAN SAMMANSLAGET HÄR, INTE I KLIENTEN: ADR-125 § 4
// kräver EN renderare — samma fallback-regel FÅR inte tolkas på två ställen
// (klient OCH renderare). Den regeln äger denna läsväg, en gång, här.
//
// SISTA BETALNINGSDAG är den ENDA härledda (aldrig lagrade) standarden:
// Startdatum − 14 dagar (ADR-125 § 2, samma regel som
// `Anmälningar.Deadline slutbetalning` — se data-model.md § Kända fällor 52
// för VARFÖR den regeln inte återanvänds rakt av: den formeln har en egen,
// orelaterad bugg i sin "Ej relevant"-undantagsgren som inte rör denna EF).
//
...
Deno.serve(async (req) => {
  ...
  try {
    // 1) Eventraden — single-get. null = 404 (ärver get-event-kontraktet).
    ...
    const responseBody = {
      ...
      kopior: {
        ...
        anmalningsavgift: standardKopia(scalarString(eif['Anmälningsavgift']), bilagetext('Anmälningsavgift')),
